...
class MLP3:
    def __init__(self, n_feature=1, n_iter=200, lr=10, tol=None, momentum = 0.9):
        ...
        self.act2 = ReLU()
        self.fc3 = Linear(32, 3, bias=True)
        self.act3 = Softmax()
...
if __name__ == '__main__':
    ...
    y_train = one_of_k_encoding(y_train,3)

    _,n_feature = X_train.shape
    model = MLP3(n_feature=n_feature,n_iter=10000,lr=4e-4,tol=1e-5)
    model.train(X_train,y_train)
    y_pred = model.predict(X_test)
    print(f"Predicted labels are{y_pred + 1}")
    print(f"Real labels are{y_test + 1}")
