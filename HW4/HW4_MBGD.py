...
class Logistic:
    ...
    def mbgd_update(self,X,y):
        ...
        for iter in range(self.n_iter):
            ...
            if(batch_size*(i+1)>X.shape[0]):
                for j in range(X.shape[0]-batch_size*i):
                    y_pred = np.append(y_pred,self._predict(X[batch_size*i+j,]))
                    #print(f"y_pred is {y_pred},y[i] is {y[i]}\n")
                    loss += self._loss(y[batch_size*i+j],y_pred[j])
                
                loss = loss/(X.shape[0]-batch_size*i)
                self.loss.append(loss)

                #print(f"x is {x}\n")
                grad = self._gradient(X[batch_size*i:X.shape[0],],y[batch_size*i:X.shape[0]],y_pred)
                ...
                #print(f"x is {x}\n")
                grad = self._gradient(X[batch_size*i:batch_size*(i+1),],y[batch_size*i:batch_size*(i+1)],y_pred)
                self.W = self.W - self.lr*grad
    ...
    def train(self,X_train,t_train):
        ...
        self.mbgd_update(X_train_bar,t_train)
        #print(f"W is {self.W}\n")
...
if __name__ == '__main__':
    ...
    model = Logistic(n_feature=n_feature,n_iter=40000,lr=2e-9,tol=1.0e-5)
