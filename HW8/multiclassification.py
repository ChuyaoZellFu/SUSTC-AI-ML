...
class Softmax:
    ...
    def forward(self, x):
        x_exp = np.exp(x - np.max(x, axis=1, keepdims=True)) # 减去最大值以提高数值稳定性
...
class MLP3:
    def __init__(self, n_feature=1, n_iter=200, lr=10, tol=None, momentum = 0.9):
        ...
        self.act2 = ReLU()
        self.fc3 = Linear(32, 10, bias=True)
        self.act3 = Softmax()
    ...
    def batch_update(self, X, y):
        ...
        for iter in range(self.n_iter):
            ...
            grad_output3 = self.act3.backward(output3a, y)
    ...
    def predict(self, X):
        ...
        return np.argmax(output3a, axis=1)
...
if __name__ == '__main__':
    data_train = pd.read_csv('HW8\optdigits.tra')
    data_test = pd.read_csv('HW8\optdigits.tes')

    X_train = data_train.iloc[:,0:63].values
    y_train = data_train.iloc[:,64].values
    y_train = one_of_k_encoding(y_train,10)
    X_test = data_test.iloc[:,0:63].values
    y_test = data_test.iloc[:,64].values

    _,n_feature = X_train.shape
    model = MLP3(n_feature=n_feature,n_iter=500,lr=1e-4,tol=None)
    ...
    np.savetxt('HW8\_test.txt', y_test, fmt='%f', delimiter=',')
    plt.figure()
    model.plot_loss()
