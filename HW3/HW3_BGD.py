...
class Perceptron:
    ...
    def _gradient(self,X,y,y_pred):
        ...
        y_pred = y_pred[:,None]
        grad = -y*X
        sign = np.tile(y_pred * y,(1,14))
        #print(f"ypred.shape is {y_pred.shape}")
        #print(f"sign.shape is {sign.shape}")
        grad[sign >=0] = 0
        return np.sum(grad,axis = 0) / len(y)
...
if __name__ == '__main__':
    ...
    model = Perceptron(n_feature=n_feature,n_iter=20000,lr=0.006,tol=1.0e-10)
